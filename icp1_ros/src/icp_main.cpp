...
class icp_node : public rclcpp::Node 
{
    ...
    private:
        ...
        void odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg)
        {
            ...
            double dx = robot_pose[0] - prev_robot_pose[0];
            double dy = robot_pose[1] - prev_robot_pose[1];
            double dtheta = robot_pose[2] - prev_robot_pose[2];
        ...
        void timer_callback()
        {
            ...
            if(flag == 0)
            {
                ...
                else
                {
                    ...
                    icp_.set_P(P);
                    icp_.set_Q(Q);
                    auto [aligned, dx, conv] = icp_.icp_function();
                    cout << "conv" <<  conv << endl;
                    cout << Q.size() << endl;
                    if (conv)
                    {
